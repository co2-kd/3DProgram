...
#include"../../GameObject/Camera/CCTVCamera/CCVTCamera.h"
...
void GameScene::Init()
{
	...
	std::shared_ptr<CCVTCamera>		_camera = std::make_shared<CCVTCamera>();
	_camera->Init();
	...
	//レッスンその3:CカードとRカードとSRカードをそれぞれ34％(Cカード)、33％（Rカード）、33％（Rカード）の確率で起動時に表示せよ！
	//{
	//	OutputDebugStringA("----------------------------------\n");
	//	int _bunbo = 100;
	...
	//			break;
	//		case 2:
	//			ss << "SRカード当選回数 =" << _TosenNum[2] << " " << "当選確率 =" << prob << "%" << "\n";
	//			break;
	//		}
	//		std::string str = ss.str();
	//		OutputDebugStringA(str.c_str());
	...
	//レッスンその4:CカードとRカードとSRカードをそれぞれ50％(Cカード)、49.5％（Rカード）、0.5％（Rカード）の確率で起動時に表示せよ！
	{
		OutputDebugStringA("----------------------------------\n");
		int _bunbo = 1000;
